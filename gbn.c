...
#define BUFFLEN 5000		//Buffer length
#define TIMEOUT_MIN 30.0	//Min value for timeout
#define TIMEOUT_MAX 60.0	//Max value for timeout
float TIMEOUT = TIMEOUT_MIN;	//Timeout value
...
int buffhead=0;	//Index pointing to location where next message would be kept
int win_base=0;	//Index pointing to base of the window
  ...
  struct msg message;
{
	...
	if(seq < win_base + WINDOW_SIZE){
		...
		printf("A:- Sending segment with seq %d to layer 3\n",packet.seqnum);
		tolayer3(A, packet);
		number_2++;
		if(seq == win_base)
		...
		printf("A:- Window full, will send the buffered segment later\n");
	}
}
  ...
  struct pkt packet;
{
	...
	else if(win_base <= ack){
		stoptimer(A);
		int steps = ack-win_base + 1;
		printf("A:- Ack %d received, moving window base by %d\n",ack,steps);
		win_base = win_base + steps;
		start_index = win_base;
	}
	if(start_index != -1){
		while(start_index < buffhead && start_index < (win_base + WINDOW_SIZE)){
			pktsent = 1;
			struct pkt nextpacket;
			nextpacket.seqnum = seq;
			int i;
			for(i=0;i<20;i++)
				nextpacket.payload[i] = buffer[seq].data[i];
			printf("A:- Sending buffered segment %d to layer 3\n",seq);
			tolayer3(A,nextpacket);
			number_2++;
		...
		printf("A:- Out of order ack %d received, dropping it\n",ack);
	}
}
...
{
	printf("A:- Timeout for segment %d\n",win_base);
	int iterator = win_base;
	starttimer(A,TIMEOUT);
	while(iterator < seq){
		struct pkt packet;
		packet.seqnum = iterator;
		int i;
		for(i=0;i<20;i++)
			packet.payload[i] = buffer[iterator].data[i];
		packet.checksum = generate_checksum(&packet);
		printf("A:- Resending segment %d\n",packet.seqnum);
		tolayer3(A,packet);
		number_2++;
		iterator++;
	}
  ...
  struct pkt packet;
{
	...
	if(verify_packet(&packet)!=0){
		...
		if(packet.seqnum == seqB){
			printf("B:- Expected packet %d received\n",seqB);
			struct msg message;
			int i;
			for(i=0;i<20;i++)
			...
			printf("B:- Out of order packet %d received, dropping it\n",packet.seqnum);
			ack.acknum = seqB - 1;
		}
	}
